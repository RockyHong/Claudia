...
describe("parseMultipart", () => {
  ...
  it("parses a valid multipart body with one file", async () => {
    ...
    const body = buildMultipartBody(BOUNDARY, [{ filename: "idle.webm", data: fileData }]);
  ...
  it("parses multiple files in one body", async () => {
    ...
    expect(files.map((f) => f.name)).toEqual(["idle.webm", "busy.webm", "pending.webm"]);
  ...
  it("rejects when no boundary in content-type header", async () => {
    const req = makeMockReq({ "content-type": "multipart/form-data" }, Buffer.alloc(0));
  ...
  it("skips parts that have no filename in their Content-Disposition header", async () => {
    ...
    const body = Buffer.from(`${noFilenamePart}${validPart}--${boundary}--\r\n`);
    const req = makeMockReq({ "content-type": CONTENT_TYPE }, body);
  ...
  it("rejects on stream error", async () => {
    ...
    const req = makeMockReqWithError({ "content-type": CONTENT_TYPE }, streamError);
